...
# 1. Proof of Concept - Cost of travel in Italy
...
# 2. Application - Cost of travel in the Mediterranean terrestrial regions 
```{r download-srtm2}
...
plot(cost_lg)
```
## Approach 2: Start with Elevation: Model cost on the entire mosaiced elevation file for the Mediterranean
...
plot(srtm_lg)
writeRaster(srtm_lg, "output_data/srtm_MED.tif", format = "GTiff")
hist(srtm_lg)
```
...
```{r}
cc
hist(cc) # distribution of travel time to nearest 1000pax town
breaks <- c(0,10,20,40,60,80,100,120)
tapply(values(cc),cut(values(cc),breaks),median)

timebins <- cut(values(cc),breaks = breaks,dig.lab = 5) ## very important to make thousands appear not as scientific notations
df_timebins <- data.frame(table(timebins))   # calculate frequency via table() and convert to dataframe for printing
df_timebins
```

## Calculate least cost path distances between inscriptions and the nearest settlement over 1000pax 

## Load the Italian inscriptions
```{r load-inscriptions}
library(sf)
library(jsonlite)
library(tidyverse)
...
# Only Italy
list_inscr_json <- jsonlite::fromJSON("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json")

# All Western Mediterranean (Claus_Slaby deduplicated) dataset 
inscrALL_json <- jsonlite::fromJSON("https://sciencedata.dk/social_diversity/e0f9cd400dd05584fe06fe1d804deff0/EDHCS_v1-0.json")

# convert json to tibble
insc <- tibble(list_inscr_json)
insc
```

Clean them up a bit and convert to sf object
```{r sf-inscr}
insc_4326 <- insc %>% 
  #select(coordinates) %>% 
  #slice(1:100) %>% 
...
mapview(insc_4326, zcol = "type_of_inscription_clean") + mapview(cities1k, cex = "pop_est", zcol = "pop_est")
```

## Calculate distance to nearest city
Distance to nearest feature is a 2 step process of first selecting the nearest town and then calculating the distance between the inscription using the conductance matrix and the nearest city.

If we were using Euclidian distance, I would use st_distance in a projected space
```{r dist-euclid}
cities1k  # should be over 1000 inhabitants and 4326 SRS
nearestcity  <- st_nearest_feature(insc_4326, cities1k$geometry)

insc_4326$Euc_dist_1ktown <- st_distance(st_transform(insc_4326,crs=3035), st_transform( cities1k[nearestcity,], crs = 3035), by_element = TRUE)

summary(insc_4326$Euc_dist_1ktown) # summary of distance in m between inscriptions and nearest town as the crow flies.

# make a map of links between inscriptions and nearest towns 
```
.. but we are using terrain / friction surface, and so we need to use costDistance() function, supply it with cost raster (conductance transition layer) and the coordinates of source, the inscription, and the destination, the nearest town of over 1000 inhabitants. 
```{r nearest-town}
# Nearest feature

nearestcitycoords  <- cities1k[st_nearest_feature(insc_4326, cities1k$geometry),]$geometry

# Cost of covering the distance in hours 

#test
insc <- as(insc_4326, "Spatial")[2,]
  nearesttown <- as(nearestcitycoords, "Spatial")[2]
  insc_4326$Lcp_dist_1ktown[2] <- costDistance(conductance,
             fromCoords= insc, 
             toCoords=nearesttown)

# prep containing df
  insc_dist <- insc_4326 %>% 
  select(id, Euc_dist_1ktown)

# full run
for (i in 1:nrow(insc_4326)){
  print(i)
  insc <- as(insc_4326, "Spatial")[i,]
  nearesttown <- as(nearestcitycoords, "Spatial")[i]
  insc_dist$Lcp_dist_1ktown[i] <- costDistance(conductance,
             fromCoords= insc, 
             toCoords=nearesttown)
  }

# alternative
ca <- accCost(x,fromCoords=A) # inscriptions
plot(ca, main = "cost of travel from  hours")
extract(ca,B) # B is a list of nearest cities

summary(insc_dist$Lcp_dist_1ktown) #multiply by 60 to get minutes

hist(insc_dist$Lcp_dist_1ktown, main= "Walking time to nearest 1000-pax-town from inscription location (in hours)")
```
# Apply the LCP calculation to all inscriptions in teh Roman Empire , not just Italy
Let's project to 2D to make the distance calculations faster. 
Start with elevation raster and regenerate cumulative cost, and calculate distances and travel times
Then, use movecost package to do the same. 
...
* van Etten J. R Package gdistance: Distances and routes on geographicalgrids. J Stat Softw 2017;76:14–5. http://dx.doi.org/10.18637/jss.v076.i13.
* EEA. European union environmental agency (E.E.A.), 2014. 2014, EU-DEM[Online].
* Tobler W. Three presentations on geographical analysis and modeling.NCGIA Tech. reports, 1, 1993, p. 1–26, https://escholarship.org/uc/item/05r820mz.
* Márquez-Pérez J, Vallejo-Villalta I, Álvarez-Francoso JI. Estimated traveltime for walking trails in natural areas. Geogr Tidsskr J Geogr2017;117:53–62. http://dx.doi.org/10.1080/00167223.2017.1316212.
* Chapa Brunet T, García J, Mayoral Herrera V, Uriarte González A.GIS landscape models for the study of preindustrial settlement pat-terns in Mediterranean areas. In: Vassilopoulos Andreas, Evelpidou Niki,Bender Oliver, Krek Alenka, editors. Geoinformation technologies forgeo-cultural landscapes. Boca Raton: CRC Press; 2008, p. 255–73.
* Pandolf KB, Givoni B, Goldman RF. J Appl Physiol 1977;43:577–81.
* Herzog I. Least-cost paths – some methodological issues. Internet Archaeology 2014. http://dx.doi.org/10.11141/ia.36.5.
